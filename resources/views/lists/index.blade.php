<x-layouts.layout titulo="Listas - Overtune">
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @foreach ($popularLists->take(4) as $list)
                <div class="card bg-primary text-white shadow-md">
                    <div class="card-body">
                        <h3 class="font-semibold text-lg">{{ $list->title }}</h3>
                        <p class="text-sm">{{ Str::limit($list->description, 100) }}</p>
                        <p class="text-xs mt-1">❤️ {{ $list->favorites_count }} favoritos</p>
                        <a href="{{ route('lists.show', $list->id) }}" class="btn btn-sm btn-secondary bg-secondary text-white hover:text-white hover:bg-orange-600 mt-2">Ver Lista</a>
                    </div>
                </div>
            @endforeach
        </div>
